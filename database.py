...
def _migrate_testrun_duplicate_names():
    """移除舊版 TestRun 名稱唯一限制，保留既有 ID 與關聯。"""
    conn = get_connection()
    try:
        unique_name_index_exists = False
        for index_row in conn.execute('PRAGMA index_list(testruns)').fetchall():
            if not index_row[2]:
                continue
            index_columns = [
                row[2]
            ...
            if index_columns == ['name']:
                unique_name_index_exists = True
                break
        if not unique_name_index_exists:
            return

        conn.execute('PRAGMA foreign_keys = OFF')
        conn.execute('BEGIN')
        conn.execute('DROP TABLE IF EXISTS testruns_duplicate_name_migration')
        conn.execute(
            '''CREATE TABLE testruns_duplicate_name_migration (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        conn.execute(
            'ALTER TABLE testruns_duplicate_name_migration RENAME TO testruns'
        )
        conn.commit()
    except Exception:
        conn.rollback()
...
def initialize_schema():
    ...
    try:
        ...
        _migrate_testrun_duplicate_names()
